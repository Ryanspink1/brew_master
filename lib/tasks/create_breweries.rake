...
task :create_breweries => :environment do
  class SeedBreweries
    ...
    def sanitize_brewery_data(parsed)
      parsed[:name] = ""                if parsed[:name].nil?
      parsed[:phone] = ""               if parsed[:phone].nil?
      parsed[:location] = {:street => "", :city => "", :state => ""}   if parsed[:location].nil?
      parsed[:emails] = ""              if parsed[:emails].nil?
      parsed[:about] = ""               if parsed[:about].nil?
      parsed[:description] = ""         if parsed[:description].nil?
      parsed[:cover] = {:source => ""}  if parsed[:cover].nil?
      parsed[:website] = ""             if parsed[:website].nil?
      parsed
    end
  end
  SeedBreweries.start
